...
class DiscordClient(discord.Client):
    ...
    def start_client(self):
        ...
        try:
            self.loop.run_until_complete(self.start(self.config.bot_token))
    ...
    async def process_loop(self):
        while not end:
            ...
            if not self.message_queue.empty():
                ...
                await self.get_channel(int(data[0])).send(data[1])
            if self.end:
                self.logger.debug("Stopping all task")
                for task in asyncio.all_tasks(self.loop):
                    task.cancel()
                self.loop.run_until_complete(await self.close())
                self.loop.close()
                self.end = False
                return

    async def on_ready(self):
        self.logger.info(f'Connected to {CLIENT_NAME}, logged on as {self.user}!')
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message.author == self.user:
            return
        if str(message.channel.id) in self.clients.keys():
            client = self.clients[str(message.channel.id)]
            msg = msg_json_formatter(client.name, message.author.name, message.content)
            message = message_formatter(client.name, message.author.name, message.content)
    ...
    def process_msg(self, msg, socket: soc.socket):
        if "action" in msg.keys():
            if msg["action"] == 'result':
                ...
                Discord_bot.message_queue.put([self.client.channel_id, message])
...
def reload():
    ...
    for i in clients.values():
        i.close_connection()
    config = init_clients(local_logger, Discord_bot.server)
    time.sleep(0.1)
    ...
    Discord_bot.stop()
    time.sleep(0.1)
    Discord_bot = DiscordClient(config, local_logger, clients, Discord_bot.server)
    threading.Thread(target=Discord_bot.start_client, name=CLIENT_NAME, daemon=True).start()
    for i in clients.values():
...
def main(server=None):
    global Discord_bot, restart_guardian, local_logger, clients
    local_logger = CBRLogger()
    ...
    for client in clients.values():
        client.try_start()
    local_logger.load()
    Discord_bot = DiscordClient(config, local_logger, clients, server)
    if auto_restart:
...
disable_duplicate_send = True  # Not recommend sending False unless you sure that your discord bot won't be banned
# if enabled, discord will send message to both full message group and less message group at same time
# if not enable, message have prefix like ##dc will not be appeared in full message group


def replace_message(msg):
    msg = msg.replace("##mc ", '').replace('##MC ', '').replace('mc ', '').replace('MC ', '')
    msg = msg.replace('##dc ', '').replace('##DC ', '').replace('dc ', '').replace('DC ', '')
    return msg
...
def on_message(server: 'CBRInterface', info: 'MessageInfo'):
    if info.client_type == CLIENT_TYPE:
        ...
        if disable_join_left and len(args) == 3 and (args[1] == 'joined' or args[1] == 'left'):
            return
        if msg.lower().startswith('##dc') or msg.lower().startswith('dc '):
            msg = replace_message(msg)
...
def on_command(server: 'CBRInterface', info: 'MessageInfo'):
    if info.content.startswith("##DC"):
        info.cancel_send_message()
        input_process(info.content.replace("##DC ", "").replace("##DC", ""))
    elif info.content.lower().startswith('##dc') or info.content.lower().startswith('dc '):
        info.cancel_send_message()

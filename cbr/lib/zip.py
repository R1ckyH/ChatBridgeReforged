...
class Compressor:
    def __init__(self, logger: CBRLogger, path: str, max_size: int, prefix: str = "") -> None:
        self.logger = logger
        self.prefix = prefix
        self.path = path
        ...
        if os.path.isfile(path):
            ...
            if file_size > self.max_size:
                ...
                with zipfile.ZipFile(zip_name, 'w') as zipper:
                    zipper.write(path, arcname=self.path, compress_type=zipfile.ZIP_DEFLATED)
                os.remove(path)
                self.logger.debug(f"Zipped {path} to {zip_name}")
            else:
